...
function filter_events() {
    ...
    $args = array(
        ...
        'posts_per_page' => -1, // Get all events for the year
        'meta_query' => array()
    );
    ...
    if ($query->have_posts()) {
        ...
        echo '<tr><td colspan="6">No events found for this year.</td></tr>';
    }
...
function add_organizer_metabox() {
    add_meta_box(
        ...
        __('Organizer Name'),       // Title of the metabox
...
function organizer_metabox_callback($post) {
    ...
    ?>
    <label for="organizer_name"><?php _e('Select Organizer:', 'text_domain'); ?></label>
...
function filter_events_by_place() {
    ...
    if ($query->have_posts()) {
        while ($query->have_posts()) {
            ...
            echo '<td>' . esc_html(get_the_title()) . '</td>';
            echo '<td>' . wp_trim_words(esc_html($details), 20, '...') . '</td>'; // Show first 20 words of details
            echo '<td>' . ($place_terms ? esc_html($place_terms[0]->name) : 'N/A') . '</td>'; // Show first place term
            echo '<td>' . esc_html($start_date) . '</td>';
            echo '<td>' . esc_html($end_date) . '</td>';
            echo '<td>' . esc_html($organizer) . '</td>';
            echo '</tr>';
        ...
        echo '<tr><td colspan="6">No events found for this place.</td></tr>';
    }
